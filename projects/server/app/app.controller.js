...
class AppController {
  ...
  async uploadDocuments(ctx) {
    ...
    let file = ctx.file
    if (!file) {
      throwApiError('The file cannot be empty.')
    }
    let buffer = [];
    const chunks = [];
    ...
    const newPath = `upload/${apiResponse.data.id}${path.extname(file.originalName)}`;
  ...
  async getDocByUploadId(ctx) {
    ...
    const resp = await sendRequestWithCatch(() => getDocByUploadIdAPI(uploadId))
    ctx.body = resp
  }
  ...
  async getDocumentToken(ctx) {
    const { id: uploadId } = ctx.params
    const token = jwt.sign({ upload_id: uploadId }, process.env.PROXY_API_KEY, { noTimestamp: true} );
    ctx.body = {
      status: 'ok',
      data: {token}
    }
